...
<body>
    ...
    <table>
        ...
        <?php $i=1; //Variable to check all info_ids $sn=1; //Variable to print the serial number while ($i<1000) { //Checking through all the info_ids $query2="
            SELECT 
              Info_ID
            FROM
              userinfo
            WHERE
              Info_ID='$i'" ; $result2=$conn->query($query2); //Execute only if a row with that info_id present in the database if($row=$result2->fetch_assoc()){ //Query to retrieve all the information from the database $query3 = " SELECT First_Name, Last_Name, Username, userinfo.Inquiry_ID, Info_ID, Age, Gender, Profession, Apt_type FROM users JOIN userinfo ON users.Inquiry_ID = userinfo.Inquiry_ID WHERE userinfo.Info_ID='$i'"; $result3 = $conn->query($query3); $row=$result3->fetch_assoc(); //Storing all the retrieved information in PHP variables $Firstname=$row["First_Name"]; $Lastname=$row["Last_Name"]; $Username=$row["Username"]; $Age=$row["Age"]; $Gender=$row["Gender"]; $Profession=$row["Profession"]; $Apt_type=$row["Apt_type"]; $info_id=$row["Info_ID"]; $Inquiry_ID=$row["Inquiry_ID"]; //Displaying all the data as a table row echo "
        <tr>
            <td>".$sn."</td>
            <td>".$Firstname."</td>
            <td>".$Lastname."</td>
        ...
        </tr>"; $sn++; //incrementing the serial number, if the row is printed } $i++; //incrementing variable to check the next info_id after each pass } ?>
    </table>
</body>
