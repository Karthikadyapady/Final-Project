...
VALUES ('$Firstname2', '$Lastname' , '$Username')" ; if ($conn->query($sql) ===TRUE) { //echo "New record created successfully
            ...
            <br>" . $conn->error; } $sql2 = "SELECT ID from users where Username='$Username'"; $result2 = $conn->query($sql2); $new_id=$result2->fetch_assoc(); //Capturing the rest of the info from the form and storing them in php variables $ID = $new_id['ID']; $Age = $_POST['Age']; $Profession = $_POST['Profession']; $Firstname2=htmlspecialchars($Firstname, ENT_QUOTES); $Gender = $_POST['Gender']; $Apt_type = $_POST['Apt_type']; //Inserting the values stored in the php variables into the inquiry2 table $sql = "INSERT INTO userinfo (ID, Age, Gender, Profession, Apt_type) VALUES ('$ID' , '$Age', '$Gender' , '$Profession', '$Apt_type')"; if ($conn->query($sql) ===TRUE) { echo "New record created successfully
            <br>"; } else { echo "Error:" . $sql . "
            <br>" . $conn->error; } //Retrieving information from both the tables over the common ID $query3 = " SELECT First_Name, Last_Name, Username, Age, Gender, Profession, Apt_type FROM users JOIN userinfo ON users.ID = userinfo.ID WHERE userinfo.ID='$ID'"; $result3 = $conn->query($query3); $row=$result3->fetch_assoc(); //Storing the information in PHP variables $Firstname=$row["First_Name"]; $Lastname=$row["Last_Name"]; $Username=$row["Username"]; $Age=$row["Age"]; $Gender=$row["Gender"]; $Profession=$row["Profession"]; $Apt_type=$row["Apt_type"]; $conn->close(); ?>
