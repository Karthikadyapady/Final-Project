   ...
   <body>
      ...
      <table>
         ...
         <?php
            ...
            while ($i<1000) {
            //Checking through all the Inquiry_IDs
            $query2 = "
            SELECT 
              ID
            FROM
            ...
            $query3 = "
            SELECT 
              First_Name, Last_Name, Username, Inquiry_ID, Info_ID, Age, Gender, Profession, Apt_type
            FROM
              bin
             WHERE
              Inquiry_ID='$i'";
            ...
            <td><a href=perm_delete.php?Inquiry_ID=" . $Inquiry_ID  ."><button type=\"button\" class=\"btn\">Delete Permanently</button></a></td></tr>";
            $sn++; //incrementing the serial number, if the row is printed
            }
            $i++; //incrementing variable to check the next inquiry_id after each pass 
            }
